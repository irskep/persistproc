...
class TestMCPTools:
    ...
    def test_start_process_tool_direct(self, process_manager_mock, mock_subprocess):
        ...
        with patch.object(persistproc, 'process_manager', process_manager_mock, create=True):
            # Import and call the function that would be wrapped by @app.tool()
            from persistproc import create_app
            app = create_app()
            ...
            def start_process_tool(command: str, working_directory: str = None, environment: dict = None) -> str:
                """Direct implementation of start_process tool for testing."""
                try:
                    result = process_manager_mock.start_process(command, working_directory, environment)
    ...
    def test_stop_process_tool_direct(self, process_manager_mock, mock_killpg, mock_getpgid):
        ...
        with patch.object(persistproc, 'process_manager', process_manager_mock, create=True):
            ...
            def get_process_log_paths_tool(pid: int) -> str:
                ...
                try:
                    ...
                    log_paths = process_manager_mock.log_manager.get_log_paths(p_info.log_prefix)
    ...
    def test_restart_process_tool_direct(self, process_manager_mock, mock_subprocess, mock_killpg, mock_getpgid):
        ...
        with patch.object(persistproc, 'process_manager', process_manager_mock, create=True):
            ...
            def restart_process_tool(pid: int) -> str:
                ...
                try:
                    ...
                    # Start new process
                    new_p_info_dict = process_manager_mock.start_process(command, wd, env)
    ...
    def test_get_process_output_tool_direct(self, process_manager_mock):
        ...
        with patch.object(persistproc, 'process_manager', process_manager_mock, create=True):
            ...
            combined_file = process_manager_mock.log_directory / "12345.echo_hello.combined"
            
            # Write some test content
            stdout_file.write_text("2023-01-01T12:00:00.000Z stdout line 1\n2023-01-01T12:00:01.000Z stdout line 2\n")
            stderr_file.write_text("2023-01-01T12:00:00.000Z stderr line 1\n")
            combined_file.write_text("2023-01-01T12:00:00.000Z stdout line 1\n2023-01-01T12:00:00.000Z stderr line 1\n2023-01-01T12:00:01.000Z stdout line 2\n")
            
            def get_process_output_tool(pid: int, stream: str, lines: int = None, before_time: str = None, since_time: str = None) -> str:
                """Direct implementation of get_process_output tool for testing."""
                try:
                    result = process_manager_mock.get_process_output(pid, stream, lines, before_time, since_time)
                    return json.dumps(result, indent=2)
                except (ValueError, RuntimeError) as e:
                    return json.dumps({"error": str(e)})
