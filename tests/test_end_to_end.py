...
import persistproc
from persistproc import ProcessManager
...
class TestEndToEnd:
    ...
    def test_mcp_server_real_process(self, temp_dir):
        ...
        pid = None
        try:
            with patch.object(persistproc, 'process_manager', pm, create=True):
                app = persistproc.create_app()
...
""")
        ...
        try:
            ...
            assert (f"CWD: {test_dir}" in stdout_content or 
                    f"CWD: /private{test_dir}" in stdout_content)
